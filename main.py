from flask import Flask, render_template, request, flash, url_for, session,jsonify, redirect
...
SMTP_PASSWORD = "pscb vctu esxy ozmx"
# Email account credentials
email_address = "adityadhanwai8@gmail.com"
password = "pscb vctu esxy ozmx"
app = Flask(__name__)
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            if user.get('is_admin', True):
                ...
                threading.Thread(target=speak, args=(response,)).start()
                return render_template('user_login_success.html')
...
def AboutUS():
    response ="College Insider is developed by Aditya Dhanwai inview of helping the students to know about the hidden facts of colleges."
    threading.Thread(target=speak, args=(response,)).start()
...
def add_review():
    ...
    code_submitted = request.form.get('verification_code', '').strip()
    code_expected  = session.pop('verification_code', None)
    expiry_iso     = session.pop('verification_expiry', None)

    # helper to return a JS-alert + redirect
    def js_alert(msg, target='/share'):
        return f'''
        <script>
          alert({msg!r});
    ...
    if not code_expected or not expiry_iso:
        return js_alert('Please request and enter a verification code.')

    # parse ISO‐timestamp
    if datetime.utcnow() > datetime.fromisoformat(expiry_iso):
        return js_alert('Your code has expired—please request a new one.')

    if code_submitted != code_expected:
        return js_alert('Incorrect verification code.')
    ...
    return render_template("success.html",
                           icon="success",
                           title="Thank you!",
                           message="Review added successfully and awaiting approval",
                           redirect_url="/user")
...
def google_auth():
    ...
    return render_template('google_success.html')
...
@admin_login_required
def approve_reviews():
    # only reviews neither approved _nor_ declined:
...
def send_verification():
    email = request.form.get('email')
    if not email:
        return jsonify(ok=False, msg="No email provided"), 400
...
def send_verification_code(recipient):
    ...
    session['verification_expiry'] = (datetime.utcnow() + VERIFICATION_TTL).isoformat()

    # send email
    subject = "Your College-Insider Verification Code"
    message = f"Your verification code is: {code}\n\nThis code will expire in {VERIFICATION_TTL.seconds//60} minutes."
    try:
        server = smtplib.SMTP(SMTP_SERVER, SMTP_PORT)
        server.starttls()
        server.login(SMTP_USERNAME, SMTP_PASSWORD)
